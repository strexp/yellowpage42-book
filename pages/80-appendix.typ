...
#body-section(
  ...
  [
    ...
    *Quick Dialing / Short Number Dialing:*
    ...
    - #text(size: 12pt)[*`N-XXXX-{SN}`*]: \ Dialing without the +042 country code prefix. Supported by some local dn42 providers for intra-network calls.

    - #text(size: 12pt)[*`{SN}`*]: \ Subscriber number only. Available on some networks when the subscriber number has a fixed length and the call remains within the same carrier's network.
